...
const (
	patchSetNonExistingNodeSelector    = `{"spec": {"template": {"spec": {"nodeSelector": {"non-existing": "true"}}}}}`
	patchRemoveNonExistingNodeSelector = `[{"op": "remove", "path": "/spec/template/spec/nodeSelector/non-existing"}]`
)
...
func (d *DaemonSetScaling) scaleDown(ctx context.Context, daemonName string) error {
	data := []byte(patchSetNonExistingNodeSelector)
	_, err := d.daemonSet.Patch(ctx, daemonName, k8stypes.StrategicMergePatchType, data, metav1.PatchOptions{})
	if err != nil {
		return fmt.Errorf("failed patching node selector spec for daemonset/%s: %w", daemonName, err)
	}
...
func (d *DaemonSetScaling) scaleUp(ctx context.Context, daemonName string) error {
	data := []byte(patchRemoveNonExistingNodeSelector)
	_, err := d.daemonSet.Patch(ctx, daemonName, k8stypes.JSONPatchType, data, metav1.PatchOptions{})
	if err != nil {
		return fmt.Errorf("failed patching node selector spec for daemonset/%s: %w", daemonName, err)
	}
